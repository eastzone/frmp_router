...
#nftest_regwrite("nf2c0", ROUTER_OP_LUT_ARP_TABLE_RD_ADDR_REG(), 0);
#nftest_regwrite("nf2c0", ROUTER_OP_LUT_ROUTE_TABLE_RD_ADDR_REG(), 0);
#nftest_regwrite("nf2c0", ROUTER_OP_LUT_DST_IP_FILTER_TABLE_RD_ADDR_REG(), 0);
...
for (my $i = 0; $i < 32; $i++){
	...
	nftest_regwrite("nf2c0", ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $i);
	my $next_hop_ip = nftest_regread("nf2c0", ROUTER_OP_LUT_GATEWAY_TABLE_ENTRY_IP_REG());
