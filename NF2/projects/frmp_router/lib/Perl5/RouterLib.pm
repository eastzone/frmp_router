#####################################
#
# $Id: RouterLib.pm 44 2009-05-30 23:17:30Z hyzeng $
#
...
sub set_router_MAC_generic { # port, MAC, delay
  ...
  my @MAC = NF2::PDU::get_MAC_address($mac);
...
#  printf("%x\n", $next_port);
  ...
  if ($next_port == 0x01) {$reg_write->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG(), $index);}
  if ($next_port == 0x04) {$reg_write->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG(), $index<<8);}
  if ($next_port == 0x10) {$reg_write->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG(), $index<<16);}
  if ($next_port == 0x40) {$reg_write->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG(), $index<<24);}
  $reg_write->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG(), $next_port);
  $reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_ENTRY_IP_REG(), $next_IP);
  $reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_WR_ADDR_REG(), $index);
...
sub check_LPM_table_entry_generic {  # index, IP_subnet, MASK, NEXT_hop_IP, port
  ...
  $gateway_addr = $reg_read->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG());
  ...
  if ($next_port == 0x01) {$reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr);}
  if ($next_port == 0x04) {$reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr>>8);}
  if ($next_port == 0x10) {$reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr>>16);}
  if ($next_port == 0x40) {$reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr>>24);}
  $reg_read_expect->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_ENTRY_IP_REG(), $next_IP);
}
...
sub get_LPM_table_entry_generic { #table index to invalidate
  ...
  my $gateway_addr = $reg_read->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG());
  my $output_port = $reg_read->( @aux, main::ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG());

  if ($output_port == 0x1) {$reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr);}
  if ($output_port == 0x4) {$reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr>>8);}
  if ($output_port == 0x10) {$reg_write->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr>>16);}
  if ($output_port == 0x40) {$reg_write->( @aux, main::main::ROUTER_OP_LUT_GATEWAY_TABLE_RD_ADDR_REG(), $gateway_addr>>24);}

  my $next_hop = $reg_read->( @aux, main::ROUTER_OP_LUT_GATEWAY_TABLE_ENTRY_IP_REG());
